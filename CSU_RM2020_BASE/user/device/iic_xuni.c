...
#define SDA(pHandle)           NULL//HAL_GPIO_ReadPin(pHandle->GPIO_SDA, pHandle->pin_SDA)
#define SDA0(pHandle)          NULL//HAL_GPIO_WritePin(pHandle->GPIO_SDA,  pHandle->pin_SDA, 0)		//IO口输出低电平
#define SDA1(pHandle)          NULL//HAL_GPIO_WritePin(pHandle->GPIO_SDA,  pHandle->pin_SDA, 1)		//IO口输出高电平  
#define SCL0(pHandle)          NULL//HAL_GPIO_WritePin(pHandle->GPIO_SCL,  pHandle->pin_SCL, 0)		//IO口输出低电平
#define SCL1(pHandle)          NULL//HAL_GPIO_WritePin(pHandle->GPIO_SCL,  pHandle->pin_SCL, 1)		//IO口输出高电平
#define DIR_OUT(pHandle)       {}
#define DIR_IN(pHandle)        {}
...
void send_ch(SIIC_HANDLE *pHandle,uint8_t c)
{
    ...
    while(i--)
    {
        if(c & 0x80)	SDA1(pHandle);//SDA 输出数据
        else			SDA0(pHandle);
        c <<= 1;
        simiic_delay();
