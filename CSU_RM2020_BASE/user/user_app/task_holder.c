...
float p_show = 0;
void getp(void * param){
	uint8_t p_temp = TO_U32(param);
	p_show = p_temp*2.5;
	//printf("get");
}

void geti(void * param){
	uint8_t p_temp = TO_U32(param);
	p_show = p_temp*2.5;
	//printf("get");
}

extern PID_AbsoluteType_holder	pid_holder_x_speed_0;
void getd(void * param){
	uint8_t p_temp = TO_U32(param);
	pid_holder_x_speed_0.kd = (float)p_temp/255*5.0;
	//printf("get");
}
	...
	while(1)
	{
		...
		if(data && !Vision_Flag)
		{
			...
			else
			{
				...
				teset_value = holder_ctrl_data->mouse_x;
			}
...
pctrl_t yaw_pos,yaw_pos2;
float yaw_ex_v2;
float yaw_speed_mpu;
PID_AbsoluteType pos_pid;
void	task_holder_control(void* param)
{
	if(Send_Flag)
...
//	static S32 Last_yaw_v = 0;
	PID_Struct_Init_abs(&pos_pid,28,0,10,3200);//30
	...
	holder_pid_init_absolute(&pid_holder_x_speed_0, 1,  0.2,  1,  1500);		//绝对式 36 0.36
...
//			if(!MyScope_Flag)
//				yaw_ex_v = PID_Update_Absolute2(&pos_pid,holder_x_target,holder_yaw_angle);
//			else
//				yaw_ex_v = -PID_Update_Absolute2(&pos_pid,holder_scope_target,real_mpu_yaw);
//		
...
//		if(teset_value > 100 || teset_value < -100)
//			yaw_ex_v = 100;
//		else
//			yaw_ex_v = 0;
		
		//teset_value *= 0.5;
		//yaw_ex_v2 = pctrl_out(&yaw_pos2,teset_value,holder_yaw_angle);
		//yaw_ex_v = 20*yaw_ex_v2;
		long int yaw_out_temp;
		yaw_speed_mpu = -(float)mpu_yaw_speed/10;
		#if(CTRL_MODE==1) //矫正控制
		yaw_out_temp = correct_ctrl(yaw_ex_v-yaw_speed_mpu);
		#else
		yaw_out_temp = PID_Update_Yaw(yaw_ex_v,yaw_speed_mpu,&pid_holder_x_speed_0);	
		yaw_out_temp *= 35;
		#endif
